...
// Falls back to XAMPP localhost values for local development
...
// ── Site Configuration ────────────────────────────────────
// SITE_URL: set via environment variable on Render
// Locally falls back to XAMPP URL
...
if ($conn->connect_error) {
    die('<div style="font-family:sans-serif;padding:40px;text-align:center;background:#000;min-height:100vh;display:flex;align-items:center;justify-content:center;">
        <div>
            <h2 style="color:#D4AF37;font-family:Georgia,serif;">⚠ Database Connection Failed</h2>
            <p style="color:#999;">Please check your database configuration.</p>
            <p style="color:#555;font-size:13px;">Error: ' . htmlspecialchars($conn->connect_error) . '</p>
        </div>
    </div>');
}
